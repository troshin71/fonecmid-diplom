...
Процедура РассчитатьОтпускные()
	...
	|	ВТ.ДатаНачала,
	|	ВТ.ДатаОкончания,
	|	ЕСТЬNULL(БазаОклад.Результат, 0) КАК РезультатОклад,
	|	ЕСТЬNULL(БазаОклад.ОтработаноДней, 0) КАК ОтработаноДней,
	...
	// ВАЖНО: Мы получаем "развернутую" таблицу (много строк баз для одного сотрудника),
	// поэтому обход будем делать с итогами.
	
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВЫБРАТЬ", "ВЫБРАТЬ РАЗРЕШЕННЫЕ"); // Хороший тон
	// Добавляем группировку к финалу запроса:
	Запрос.Текст = Запрос.Текст + "
	|ИТОГИ
	|	СУММА(РезультатОклад),
	|	СУММА(ОтработаноДней),
	|	СУММА(РезультатПремия)
	|ПО
	|	НомерСтроки"; 
	
	РезультатЗапроса = Запрос.Выполнить();
	
	// Обходим группировку по НомеруСтроки (это наши сотрудники-отпускники)
	Выборка = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	
	Пока Выборка.Следующий() Цикл
		
		СуммаБазы = Выборка.РезультатОклад + Выборка.РезультатПремия;
		ДниБазы   = Выборка.ОтработаноДней;
		
		// Расчет
		// (Длительность отпуска * СреднийЗаработок)
		
		ДнейОтпуска = (Выборка.ДатаОкончания - Выборка.ДатаНачала) / 86400 + 1;
		...
		// Так как мы выбирали НомерСтроки, мы точно знаем, куда писать
		Движение = Движения.ВКМ_ОсновныеНачисления[Выборка.НомерСтроки - 1];
