...
Процедура РассчитатьОтпускные()
	...
	Пока Выборка.Следующий() Цикл
		...
		НоваяСтрока.РезультатОклад = Выборка.РезультатОклад;
		НоваяСтрока.ОтработаноДней = Выборка.ОтработаноДней;
		НоваяСтрока.РезультатПремия = Выборка.РезультатПремия;
	КонецЦикла;
	
	// Группируем и вычисляем
	ТаблицаИтогов = ТаблицаДанных.Свернуть("НомерСтроки, ДатаНачала, ДатаОкончания", 
		"РезультатОклад, ОтработаноДней, РезультатПремия");
	...
	Для Каждого СтрокаИтог Из ТаблицаИтогов Цикл
		...
		Если СтрокаИтог.ДатаНачала = Неопределено 
			Или СтрокаИтог.ДатаОкончания = Неопределено Тогда
			Продолжить;
		КонецЕсли;
