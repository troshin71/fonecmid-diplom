...
Запрос = Новый Запрос;
	...
	// 1. Настраиваем параметры для получения базы (за какие начисления и за какой период)
	// По заданию: База берется из Основных и Дополнительных начислений
	
	// Массив видов расчета, для которых считаем базу (только Отпуск)
	МассивВидовРасчета = Новый Массив;
	МассивВидовРасчета.Добавить(ПланыВидовРасчета.ВКМ_ОсновныеНачисления.Отпуск);
	
	// Формируем поля базы (какие ресурсы нам нужны)
	// Нам нужны: Результат (деньги) и ОтработаноДней (время)
	Поля = "ВКМ_Результат, ВКМ_ОтработаноДней"; 
	
	// Создаем запрос к виртуальной таблице "База..."
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	База.НомерСтроки КАК НомерСтроки,
		|	База.ВКМ_РезультатБаза КАК РезультатБаза,
		|	База.ВКМ_ОтработаноДнейБаза КАК ОтработаноДнейБаза,
		|	ДанныеГрафика.ЗначениеФактическийПериодДействия КАК ДнейОтпуска
		|ИЗ
		|	РегистрРасчета.ВКМ_ОсновныеНачисления.БазаОсновныеНачисления(
		|			&ПериодРегистрации,
		|			&ПериодРегистрации,
		|			ВидРасчета В (&МассивВидовРасчета),
		|			&ПоляБазы) КАК База
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрРасчета.ВКМ_ОсновныеНачисления.ДанныеГрафика(
		|				Регистратор = &Регистратор
		|					И ВидРасчета В (&МассивВидовРасчета)) КАК ДанныеГрафика
		|		ПО База.НомерСтроки = ДанныеГрафика.НомерСтроки";
		
	// Параметры базы
	Запрос.УстановитьПараметр("МассивВидовРасчета", МассивВидовРасчета);
	Запрос.УстановитьПараметр("ПоляБазы", Поля);
	Запрос.УстановитьПараметр("Регистратор", Ссылка);
	...
	Пока Выборка.Следующий() Цикл
		...
		// Формула: (БазаДеньги / БазаДни) * ДнейОтпуска
		
		СуммаБазы = ?(Выборка.РезультатБаза = NULL, 0, Выборка.РезультатБаза);
		ДниБазы   = ?(Выборка.ОтработаноДнейБаза = NULL, 0, Выборка.ОтработаноДнейБаза);
		
		Если ДниБазы = 0 Тогда
			СтрокаДвижения.ВКМ_Результат = 0;
		Иначе
			СтрокаДвижения.ВКМ_Результат = (СуммаБазы / ДниБазы) * Выборка.ДнейОтпуска;
		КонецЕсли;
