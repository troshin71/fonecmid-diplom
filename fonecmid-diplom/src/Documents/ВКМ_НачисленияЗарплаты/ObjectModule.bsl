...
// 1. Создаем пустые движения
	...
	// Записываем, чтобы создать структуру записей
	Движения.ВКМ_ОсновныеНачисления.Записать();
	
	// 2. Считаем ОТПУСК
	РассчитатьОтпускные();
	// Обязательно записываем! Иначе механизм вытеснения не узнает о периоде отпуска
	Движения.ВКМ_ОсновныеНачисления.Записать(); 
	...
	// Перед тем как считать НДФЛ и Взаиморасчеты, нужно записать ВСЕ начисления в базу.
	// Иначе запрос в процедуре РассчитатьНДФЛ вернет пустоту.
	
	Движения.ВКМ_ОсновныеНачисления.Записать();
	Движения.ВКМ_ДополнительныеНачисления.Записать();
	
	// 5. Теперь считаем НДФЛ (он увидит записанные данные)
	РассчитатьНДФЛ();
	
	// Записываем НДФЛ перед расчетом выплаты (чтобы вычесть налог из суммы к выплате)
	Движения.ВКМ_Удержания.Записать();
	
	// 6. Заполняем Взаиморасчеты
...
Процедура РассчитатьПремии()
	...
	|	ТЧ.Сотрудник КАК Сотрудник,
	|	ТЧ.ВидРасчета КАК ВидРасчета,
	|	ЕСТЬNULL(ВКМ_ВыполненныеСотрудникомРаботыОбороты.ВКМ_СуммаКОплатеОборот, 0) КАК СуммаПремии
	|ИЗ
...
Процедура РассчитатьОтпускные()
	...
	|		ПО ВТ.Сотрудник = БазаПремия.ВКМ_Сотрудник
