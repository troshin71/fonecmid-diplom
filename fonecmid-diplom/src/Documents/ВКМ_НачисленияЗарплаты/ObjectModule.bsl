...
Процедура РассчитатьОклад()
	...
	Пока Выборка.Следующий() Цикл
		...
		Движение.ВКМ_ОтработаноЧасов = Выборка.Отработано;
		
		Если Движение.Сторно Тогда
			Движение.ВКМ_Результат = -Движение.ВКМ_Результат;
			Движение.ВКМ_ОтработаноЧасов = -Движение.ВКМ_ОтработаноЧасов;
		КонецЕсли;
...
Процедура РассчитатьОтпускные()
	...
	|	ЕСТЬNULL(БазаОклад.ВКМ_ОтработаноЧасов, 0) КАК ОтработаноЧасов,
	...
	// Здесь простой выбор, так как группировки в запросе нет
	...
	ТаблицаДанных.Колонки.Добавить("ВКМ_ОтработаноЧасов");
	...
	// !!! ИСПРАВЛЕНИЕ: Свернуть меняет таблицу САМУ, а не возвращает новую !!!
	ТаблицаДанных.Свернуть("НомерСтроки, ДатаНачала, ДатаОкончания", "РезультатОклад, ВКМ_ОтработаноЧасов, РезультатПремия");
	
	// Перебираем уже свернутую таблицу
	Для Каждого СтрокаИтог Из ТаблицаДанных Цикл
		
		СуммаБазы = СтрокаИтог.РезультатОклад + СтрокаИтог.РезультатПремия;
		ДниБазы   = СтрокаИтог.ОтработаноЧасов;
		
		// Проверка на корректность дат
		Если Не ЗначениеЗаполнено(СтрокаИтог.ДатаНачала) 
			Или Не ЗначениеЗаполнено(СтрокаИтог.ДатаОкончания) Тогда
			Продолжить;
		КонецЕсли;
